...
mod tests {
    ...
    #[test]
    fn deserialize_inline_gzip_entry() {
        use base64::Engine;
        use base64::engine::general_purpose::STANDARD as BASE64;
        ...
        let json = format!(
            r#"{{"version":1,"entries":[{{"path":"f.txt","data":"{}","encoding":"gzip+base64"}}]}}"#,
            encoded
        );
        let manifest: Manifest = serde_json::from_str(&json).unwrap();
        assert_eq!(manifest.entries[0].inline_data(), Some(original.as_slice()));
        assert_eq!(manifest.entries[0].known_size(), Some(12));
    }

    #[test]
    fn deserialize_rejects_invalid_gzip() {
        let json = r#"{"version":1,"entries":[{"path":"f.txt","data":"bm90IGd6aXA=","encoding":"gzip+base64"}]}"#;
        assert!(serde_json::from_str::<Manifest>(json).is_err());
    }
